    ...
    private static void outputPoints(double[] inputX, double[] inputY, int[] inputZ) {
        ...
        for (int i = 0; i < inputX.length; i++) {
             System.out.println(String.format(
                    ...
                    i<inputX.length-1?inputX[i+1]:inputX[0], i<inputX.length-1?inputY[i+1]:inputY[0], zzero, //координаты x; y; z; точки P(I+1). У последнего треугольника точка P(i+1) совпадает с точкой P(i) первого треугольника, поэтому чтобы java не ругалась на обращение к не существующему элементу массива, пришлось применть терналнальный оператор.
